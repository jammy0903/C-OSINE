      ...
      <div className={`flex flex-col ${selectedProblem ? 'w-1/2' : 'w-full'}`}>
        ...
        {isTerminalOpen && (
          <ScrollArea className="h-1/2 px-5 py-4">
            {/* Judge Results */}
            {judgeResults && (
              <JudgeResultsView results={judgeResults} />
            )}

            {/* Run Result */}
            {!judgeResults && (
              <RunResultView result={result} isRunning={isRunning} isJudging={isJudging} />
            )}
          </ScrollArea>
        )}
      </div>
...
// 채점 결과 뷰
function JudgeResultsView({ results }: { results: { results: { passed: boolean; input?: string; expected?: string; actual?: string; time?: string }[]; allPassed: boolean; passedCount: number; totalCount: number } }) {
  return (
    <div className="mb-4">
      <div className="flex items-center gap-3 mb-3">
        <span className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Result</span>
        <Badge variant={results.allPassed ? 'default' : 'destructive'} className={results.allPassed ? 'bg-emerald-600' : ''}>
...
// 실행 결과 뷰
function RunResultView({ result, isRunning, isJudging }: { result: { success: boolean; output: string; time?: string } | null; isRunning: boolean; isJudging: boolean }) {
  return (
    <>
      <div className="flex items-center gap-3 mb-3">

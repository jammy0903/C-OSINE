...
import { getChangedBlocks, getNewBlocks, getRemovedBlocks } from '../utils';
...
export function useStepTransition(currentStep: Step | null) {
  ...
  useEffect(() => {
    ...
    // Future: newBlocks and removedBlocks for animations
    getNewBlocks(prevStep, currentStep);
    getRemovedBlocks(prevStep, currentStep);
